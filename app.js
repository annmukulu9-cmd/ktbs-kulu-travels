...
function openQuotation(id){
  ...
  setTimeout(()=>{
    ...
    box.className='card';
    box.style.cssText='margin:12px 0;box-shadow:none';
    box.innerHTML=`
      <div class="kicker">ACCOUNTABILITY</div>
      <b>Assigned Consultant</b>
      <div style="margin-top:4px">
        ${esc(profileName(q.consultant_id))}
      </div>

      <div style="margin-top:12px">
        <b>Last Updated By</b>
        <div style="margin-top:4px">
          ${esc(profileName(q.updated_by))}
        </div>
      </div>
    `;
...
function bookingTable(n=999,status=''){
  const arr=cache.bookings.filter(b=>!status||b.status===status);
  ...
  return `<table class="booking-table">
    ...
    <tbody>
      ${arr.slice(0,n).map(b=>{
        ...
        return `<tr>
          <td>
            <button class="link-btn" onclick="openBooking('${b.id}')">Open</button>
          </td>
          <td>
            <b>${esc(b.booking_no)}</b><br>
            <small>${esc(quoteById(b.quotation_id)?.quotation_no||'')}</small>
          </td>
          <td>
            ${esc(c?.name||'—')}<br>
            <small>${esc(c?.client_code||'')}</small>
          </td>
