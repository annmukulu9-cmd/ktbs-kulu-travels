...
const STATUS=['Quotation','Confirmed','Partially Paid','Fully Paid','In Progress','COMPLETED','Cancelled'];
...
async function newClient(existing=null){
 const c=existing||{name:'',phone:'',email:'',nationality:'Kenyan',passport:'',destination:'',departure:'',return_date:'',adults:1,children:0,budget:0,lead_source:'Website',consultant_id:me?.id||'',status:'Quotation',remarks:''};
...
async function saveServiceFromQuote(){const name=($('quoteNewService')?.value||'').trim();if(!name)return alert('Enter a service or travel item name first.');const exists=cache.services.some(s=>s.name.toLowerCase()===name.toLowerCase());if(exists)return alert('That service already exists.');const r=await sb.from('services').insert({name,active:true,created_by:me.id});if(r.error)return alert(r.error.message);await loadData();$('quoteItems').insertAdjacentHTML('beforeend',quoteRow({service_name:name}));$('quoteNewService').value='';alert('Service saved to the master list and added to this quotation.');calcQuote()}
